...
(async () => {
  ...
  if ((await page.locator(".word-unit").count()) !== 289) throw new Error("word count mismatch");
  ...
  if (data.voice !== "en-GB-RyanNeural" || data.wordCount !== 289) throw new Error("metadata mismatch");
  ...
  console.log(JSON.stringify({ ok: true, words: 289, voice: data.voice, screenshots: 2 }, null, 2));
})().catch((error) => {
