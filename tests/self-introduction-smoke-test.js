...
(async () => {
  ...
  if ((await page.locator(".word-unit").count()) !== 288) throw new Error("word count mismatch");
  ...
  if (data.voice !== "en-GB-RyanNeural" || data.wordCount !== 288) throw new Error("metadata mismatch");
  ...
  console.log(JSON.stringify({ ok: true, words: 288, voice: data.voice, screenshots: 2 }, null, 2));
})().catch((error) => {
